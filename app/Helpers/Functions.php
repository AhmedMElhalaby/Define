...
class Functions
{
    ...
    public static function SendNotification($user,$title,$msg,$title_ar,$msg_ar,$ref_id = null,$type= 0,$store = true,$replace =[]): bool
    {
        ...
        if ($user->device_token){
            ...
            $message = array
            (
                'body'  => ($user->getAppLocale() == 'en')?$msg:$msg,
                'title' => ($user->getAppLocale() == 'en')?$title:$title,
                'sound' => true,
...
//        }
    ...
    public static function Translation($ref_id,$column,$model_type){
        $language = Language::where('code',app()->getLocale())->first();
        $ObjectTranslation = \App\Models\Translation::where('ref_id','=',$ref_id)->where('language_id','=',$language->id)->where('column','=',$column)->where('model_type','=',$model_type)->first();
