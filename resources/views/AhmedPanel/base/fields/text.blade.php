<div class="col-md-12" data-name-filed="{{$Field['name']}}">
    <div class="{{isset($Field['translate'])  ? 'col-md-10':' col-md-12'}}">
        <div class="form-group label-floating">
            ...
            <input type="text" id="{{$Field['name']}}" name="{{$Field['name']}}"
                   ...
                   @endif class="form-control {{ $errors->has($Field['name']) ? ' is-invalid' : '' }}"
                   value="{{$value}}">
        </div>
                                ...
                                <select name="Language[{{$Field['name']}}][]" style="margin: 0;padding: 0"
                                    ...
                                    @foreach(\App\Models\Language::all() as $Language)
                                        @if($Language->code != 'en')
                                            ...
                                            <option value="{{$Language->id}}" {{$Language->id}} data-s="{{$languages->id }}"
                                                {{$Object !=  null && $Language->id == $languages->id &&  $ObjectTranslation  ? 'selected':'' }}
                        ...
                        <div class="col-md-5">
                            <div class="form-group label-floating">
                                ...
                                <input type="text" id="{{$Field['name']}}" name="{{$Field['name']}}_translate[]"
                                       ...
                                       value="{{$Object !=  null && $ObjectTranslation  ? $ObjectTranslation->translation : ''}}">
                            </div>
                        </div>
                        <div class="col-md-2">
