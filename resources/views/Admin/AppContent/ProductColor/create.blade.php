                                ...
                                <div class="col-md-12" id="Roles">
                                    ...
                                    @foreach (\App\Models\Role::all() as $role)
                                        <div class="col-md-3">
                                            ...
                                            <label
                                                    for="{{ $role->getId() }}">{{ app()->getLocale() == 'ar' ? $role->getNameAr() : $role->getName() }}</label>
                                        </div>
                                        <script>
                                            @foreach ($role->role_permissions as $role_permission)
                                            {{ $role_permission->permission_id }} @if (!$loop->last),@endif
                                            @endforeach]
                                            ;
                                        </script>
                                    @endforeach
                                ...
                                <div class="col-md-12" id="Permission">
                                    ...
                                    @foreach (\App\Models\Permission::whereNull('parent_id')->get() as $permission)
                                        <div class="form-group col-md-12">
                                            ...
                                            <label
                                                    for="permission{{ $permission->getId() }}">{{ app()->getLocale() == 'ar' ? $permission->getNameAr() : $permission->getName() }}</label>
                                        </div>
                                        @foreach (\App\Models\Permission::where('parent_id', $permission->getId())->get() as $cPermission)
                                            <div class="form-group col-md-3">
                                                ...
                                                <label
                                                        for="permission{{ $cPermission->getId() }}">{{ app()->getLocale() == 'ar' ? $cPermission->getNameAr() : $cPermission->getName() }}</label>
                                            </div>
                                        @endforeach
                                    @endforeach
                                </div>
    ...
    <script>
        ...
        $('#new_option_quantities').on('click', function () {
            ...
            $('#quantities').append(' <div id="quantities_row' + i + '">' +
                    ...
                    '</div>' +
                    '<div class="col-md-1">' +
                    '<div class="btn btn-danger no-text ticket-manage remove_field_quantities" id="' + i + '" >حذف </div>  </div>' +
                    ' </div>');
        });
...
//                    '</div>' +
//                    '<div class="col-md-1">' +
//                    '<div class="btn btn-danger no-text ticket-manage remove_field_logos"  id="' + x + '" >حذف</div>  </div>' +
//                    ' </div>');
//        });
...
//        });
        ...
        $('#new_option_colors').on('click', function () {
            ...
            $('#colors').append(' <div id="colors_row' + y + '">' +
                    ...
                    '</div>' +
                    '<div class="col-md-1">' +
                    '<div class="btn btn-danger no-text ticket-manage remove_field_colors"  id="' + y + '" >حذف </div>  </div>' +
                    ' </div>');
        });
